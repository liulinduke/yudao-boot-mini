...
namespace SocialMatrix.WpfHost.Services
{
    ...
    public class PublishPostScriptBuilder : FacebookScriptBuilder
    {
        ...
        private void AppendSetPrivacySteps(int privacySetting)
        {
            ...
            _js.AppendLine("                const currentPrivacy = normalizeText(privacyButton.innerText || privacyButton.getAttribute('aria-label') || '');");
            _js.AppendLine("                if (privacyKeywords.some(keyword => currentPrivacy === keyword || currentPrivacy.includes('Sharing with ' + keyword) || currentPrivacy.includes('分享给' + keyword))) {");
            _js.AppendLine("                    console.log('[发个人帖] 隐私已是目标值，跳过设置');");
            _js.AppendLine("                } else {");
        ...
        private void AppendInputContentSteps(string content)
        {
            ...
            _js.AppendLine("            const editorText = () => String(textbox.innerText || textbox.textContent || '')");
            _js.AppendLine("                .replace(/\\r\\n/g, '\\n').replace(/\\u200b/g, '').replace(/^\\n+|\\n+$/g, '');");
            _js.AppendLine("            const existingText = editorText();");
            _js.AppendLine("            if (existingText === content) {");
            _js.AppendLine("                console.log('[发个人帖] 内容已存在，跳过重复输入', { contentLength: content.length });");
            _js.AppendLine("                return JSON.stringify({ success: true, skipped: true });");
            _js.AppendLine("            }");
            _js.AppendLine("            if (window.__socialMatrixPublishInputInProgress) {");
            _js.AppendLine("                console.log('[发个人帖] 已有输入任务执行中，跳过重复调用');");
            _js.AppendLine("                return JSON.stringify({ success: true, skipped: true });");
            _js.AppendLine("            }");
            _js.AppendLine("            window.__socialMatrixPublishInputInProgress = true;");
            ...
            _js.AppendLine("            selectEditorContents();");
            _js.AppendLine("            // 与私信发送一致：逐字输入并加入不规则停顿。每次 execCommand 会触发对应 input 事件。");
            ...
            _js.AppendLine("            }");
            _js.AppendLine("            await randomDelay(800, 1500);");
            _js.AppendLine("            const actualText = editorText();");
            _js.AppendLine("            if (actualText !== content) {");
            _js.AppendLine("                throw new Error('帖子内容输入结果不一致，已停止发布');");
            _js.AppendLine("            }");
            _js.AppendLine("            console.log('[发个人帖] 帖子内容输入完成，仅执行一次', { contentLength: content.length });");
            _js.AppendLine("            window.__socialMatrixPublishInputInProgress = false;");
            _js.AppendLine("            await blurActiveEditor();");
            _js.AppendLine("");
        }
        ...
        private void AppendClickPostSteps()
        {
            ...
            _js.AppendLine("                const composer = getComposerDialog();");
            _js.AppendLine("                const roots = composer ? [composer] : Array.from(document.querySelectorAll('[role=\"dialog\"]'));");
            _js.AppendLine("                const buttons = roots.flatMap(root => Array.from(root.querySelectorAll('[role=\"button\"]')));");
            _js.AppendLine("                return buttons.find(button => {");
            ...
            _js.AppendLine("                    return /^(post|publish|发帖|发布)$/i.test(label) || button.getAttribute('data-testid') === 'react-composer-post-button';");
            _js.AppendLine("                });");
            _js.AppendLine("            };");
